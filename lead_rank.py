...
def lead_of(x: pd.Series, y: pd.Series, groups: pd.Series, episode: pd.Series,
    ...
    # --- Diagnostic explicite. Renvoyer None en silence masque la cause reelle,
    # qui est presque toujours un desalignement d'index ou un grp inexistant.
    if not isinstance(x.index, (pd.PeriodIndex, pd.DatetimeIndex)):
        raise TypeError("x doit avoir un index temporel (PeriodIndex/DatetimeIndex), "
                        f"recu {type(x.index).__name__}.")
    if not isinstance(y.index, (pd.PeriodIndex, pd.DatetimeIndex)):
        raise TypeError("y doit avoir un index temporel, "
                        f"recu {type(y.index).__name__}.")
    dispo = sorted(pd.Series(groups).dropna().unique().tolist())
    if grp is not None and grp not in dispo:
        raise ValueError(
            f"grp={grp!r} est absent de `groups`. Valeurs disponibles : {dispo}. "
            "Verifiez que `groups` a bien ete construit avec le meme "
            "apply_grouping(...) que la valeur passee en grp.")
    inter = groups.index.intersection(x.index).intersection(y.index)
    if len(inter) == 0:
        raise ValueError(
            "Aucune date commune entre `groups`, `x` et `y`. Les trois doivent "
            f"partager un index de meme frequence. groups={groups.index.dtype}, "
            f"x={x.index.dtype}, y={y.index.dtype}.")
    if len(episode) != len(groups):
        raise ValueError(
            f"`episode` ({len(episode)}) et `groups` ({len(groups)}) n'ont pas la "
            "meme longueur. Construisez les deux depuis le MEME DataFrame phases "
            "(memes lignes ecartees, par exemple 'Choc Covid').")
    ...
    for k in ks:
        ...
        if r is not None:
            fits[k], dfs[k] = r, d
    if len(fits) < 2:
        d0 = _panel_for(x, y, groups, ks[0], episode)
        d0 = d0[d0.g == grp] if grp is not None else d0
        raise ValueError(
            f"Moins de 2 horizons estimables pour grp={grp!r}. "
            f"Apres alignement des dates il reste {len(d0)} observations "
            f"(minimum requis : min_n={min_n}). "
            + ("Cause probable : la serie x commence trop tard, ou le "
               "sous-groupe est trop petit." if len(d0) < min_n else
               "Cause probable : x est constante sur ce sous-echantillon."))
