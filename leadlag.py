...
# --- 5. Alignement calendaire -------------------------------------------------
print("\n" + "=" * 72)
print("5. ALIGNEMENT SUR LES DATES — l'avance vraie est +6 trimestres")
print("=" * 72)
g = np.random.default_rng(3)
n = 240
idx = pd.period_range("1965Q1", periods=n, freq="Q")
xq = ar1(n, .85, g)
yq = np.roll(xq, 6) + g.normal(0, 1.1, n)
X, Y = pd.Series(xq, index=idx), pd.Series(yq, index=idx)

print("\n(a) series completes")
print(leadlag_ccf(X, Y, max_lag=12, n_boot=400).summary())

print("\n(b) 15 trimestres manquants au milieu de X — le pic ne doit pas bouger")
Xg = X.copy(); Xg.iloc[100:115] = np.nan
print(leadlag_ccf(Xg, Y, max_lag=12, n_boot=400).summary())

print("\n(c) supports decales : X depuis 1965, Y depuis 1975")
print(leadlag_ccf(X, Y.iloc[40:], max_lag=12, n_boot=400).summary())

print("\n(d) garde-fous")
Xm = pd.Series(g.normal(size=400), index=pd.period_range("1990-01", periods=400, freq="M"))
for lab, args in [("frequences M vs Q", (Xm, Y)), ("serie datee vs ndarray nu", (X, yq))]:
    try:
        leadlag_ccf(*args)
        print(f"  {lab:28s} : NON REJETE (probleme)")
    except ValueError as e:
        print(f"  {lab:28s} : rejete — {str(e)[:58]}...")
